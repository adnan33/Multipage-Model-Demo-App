...
def cloth_segmentation_process(img,output_shape):
  ...
  if output_shape:
      st.image(img.resize(img_shapes.cloth_image_output_shape), caption = "Input Cloth Image", channels = "RGB")
  else:
      st.image(img, caption = "Input Cloth Image", channels = "RGB")
  output = get_cloth_segmask(img)
  output = Image.fromarray((output*255).astype(np.uint8))
  st.markdown("<h3 style=' color: #666;'>Predicted Segmentation Mask</h3>", unsafe_allow_html=True)
  if output_shape:
      st.image(output.resize(img_shapes.cloth_image_output_shape), caption = "Predicted Cloth Segmentation Mask" , channels = "RGB")
  else:
...
def human_parsing_segmentation_process(img,output_shape):
  ...
  if output_shape:
      ...
      st.image(img.resize(img_shapes.hp_output_shape), caption = "Input image", channels = "RGB")
  output_mask = get_hp_colormask(img)
  st.markdown("<h3 style=' color: #666;'>Predicted Segmentation Mask</h3>", unsafe_allow_html=True)
  if output_shape:
      st.image(output_mask, caption = "Predicted Segmentation Mask", channels = "RGB")
  else:
      st.image(output_mask.resize(img_shapes.hp_output_shape), caption = "Predicted Segmentation Mask", channels = "RGB")
  output = get_colormask_overlay(img,output_mask)
  st.markdown("<h3 style=' color: #666;'>Predicted Segmentation Mask Overlay</h3>", unsafe_allow_html=True)
  if output_shape:
      st.image(output, caption = "Predicted Segmentation Mask Overlayed", channels = "RGB")
  else:
      st.image(output.resize(img_shapes.hp_output_shape), caption = "Predicted Segmentation Mask Overlayed", channels = "RGB")
